...
function SWEP:UseActivate()
	...
	sound.Play("snd_jack_hmcd_bandage.wav", self:GetOwner():GetShootPos(), 60, math.random(90, 100))
	self:GetOwner():ViewPunch(Angle(-10, 0, 0))
	self:GetOwner().Bleedout = math.Clamp(self:GetOwner().Bleedout - 40, 0, 1000)
...
function SWEP:UseOthers(ply, pos)
	if IsValid(ply) and ply:IsPlayer() and (ply.Bleedout > 0) then
		...
		ply.Bleedout = math.Clamp(ply.Bleedout - 60, 0, 1000)
		ply:RemoveAllDecals()
		if SERVER then
			self:Remove()
		end
