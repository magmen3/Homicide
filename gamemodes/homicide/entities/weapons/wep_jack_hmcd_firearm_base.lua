...
function SWEP:PrimaryAttack()
	...
	if not (self:GetAiming() > 99) then
		InAcc = InAcc + self.HipFireInaccuracy
	end
...
function SWEP:BarrelSmoke()
	...
	if CLIENT then
		...
		if ent then
			ParticleEffectAttach("pcf_jack_mf_barrelsmoke", PATTACH_POINT_FOLLOW, ent, 2)
		end
	else
		for i = 0, math.random(1, 2) do
			timer.Simple(
				i / 2,
				function()
					if IsValid(self) and self:GetOwner() and self:GetOwner().Alive and self:GetOwner():Alive() then
						ParticleEffectAttach("pcf_jack_mf_barrelsmoke", PATTACH_POINT_FOLLOW, self:GetOwner(), self:GetOwner():LookupAttachment("anim_attachment_RH"))
					end
				end
			)
		end
...
if CLIENT then
	...
	function SWEP:GetViewModelPosition(pos, ang)
		...
		if self.CloseAimPos and (Aim > 0) then
			Vec = Vec + self.CloseAimPos * ExtraAim
		end
