...
function GM:RenderRespawnText()
	local client = LocalPlayer()
	local sw, sh = ScrW(), ScrH()
	if client.ForgiveTime and (client.ForgiveTime > CurTime()) then draw.DrawText(translate.wrongfullyKilled, "MersRadialSemiSuperS", sw / 2, sh - 150, Color(255, 255, 255, 128), 1) end
end

net.Receive("hmcd_forgive", function() LocalPlayer().ForgiveTime = CurTime() + 10 end)