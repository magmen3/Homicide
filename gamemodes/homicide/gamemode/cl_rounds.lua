...
net.Receive("DeclareWinner", function(length)
	...
	while true do
		...
		if IsValid(t.player) then t.playerName = t.player:Nick() end
	...
	if IsValid(LocalPlayer()) then LocalPlayer():EmitSound("ambient/alarms/warningbell1.wav", 100, pitch) end
end)
