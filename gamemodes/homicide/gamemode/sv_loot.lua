...
local SHTF_LootTable = {
	...
	{
		"ent_jack_hmcd_molotov",
		.5
	},
	{
		"ent_jack_hmcd_pipebomb",
		.5
	},
	{
...
function GM:SpawnLoot(SpawnPos, noJunk, goodShit)
	...
	if SpawnPos then
		...
		for key, item in pairs(AllObjects) do
			...
			if table.HasValue(HMCD_JunkLootModels, Maud) then GoodJunk = GoodJunk + 1 end
		end
...
function GM:LootThink()
	if self:GetRound() == 1 then
		...
		if not self.LastSpawnLoot or self.LastSpawnLoot < CurTime() then
			...
			if Amt < (40 + 5 * #team.GetPlayers(2)) then self:PutLootOnMap() end
		end
	end
end
...
function GM:PutLootOnMap()
	...
	for key, item in pairs(Objs) do
		if table.HasValue(HMCD_ContainerModels, string.lower(item:GetModel())) and not item.LootFilled then table.ForceInsert(Containers, item) end
	end
