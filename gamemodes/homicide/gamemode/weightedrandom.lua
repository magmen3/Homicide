...
-- this initializes any new players with the current averages
function WRandom:FindSetDefaultAverages()
...
-- now dis is some magic shit hier, son
function WRandom:Roll()
	local Playas, PotentialKillas, PotentialGunnas, Killa, Gunna = team.GetPlayers(2), {}, {}, 0, nil
	self:FindSetDefaultAverages()
	for key, playa in pairs(Playas) do
		local Record = GAMEMODE.SHITLIST[playa:SteamID()] or 0
		if GAMEMODE.GuiltDisabled:GetInt() == "1" then Record = 0 end
		if Record < GAMEMODE.GimpPunishmentThreshold or GAMEMODE.GuiltDisabled:GetBool() then
	...
	for key, playa in pairs(Playas) do
		local Record = GAMEMODE.SHITLIST[playa:SteamID()] or 0
		if not ((Record >= GAMEMODE.GimpPunishmentThreshold) and not GAMEMODE.GuiltDisabled:GetBool()) or playa == Killa then
	...
	if not Killa then
		print("Homicide: murderer selection failed!")
		Killa = Playas[1]
	end

	if not Gunna then
		print("Homicide: gunman selection failed!")
		Gunna = Playas[2]
	end
	...
	for key, playa in pairs(Playas) do
		...
		local Record = GAMEMODE.SHITLIST[playa:SteamID()] or 0
		if (Record >= GAMEMODE.GimpPunishmentThreshold) and not GAMEMODE.GuiltDisabled:GetBool() then
